...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def getFile(self):
        ...
        if ' ' in self.filePathOut:
            msg = QtWidgets.QMessageBox()
            msg.setIcon(QtWidgets.QMessageBox.Information)
            msg.setText("Please do not use paths with spaces. Correct and try again")
        ...
        with open('dataProcessingLogs.txt', 'w') as f:
            try:
                ...
                if isBlenderHere[0].exists():
                    ...
                    f.write('Failed to find Blender.exe : ' + str(inst))
                ...
                # safe input check
                self.percent = float(self.CleanUp_percentage.text())

                if self.headless == '-b':
                    args = "{} -b -P RMIT_BlenderDriver.py -- -i {} -e {} -d {} -c {} -l {} -x {}".format(
                        blenderPath,self.filePathIn,self.filePathOut,self.decimateAmount,self.cleanUp,str(self.loose),self.percent)
                else:
                    # Blender recognizes all arguments after -- as args for scripts.
                    args = "start cmd.exe /k {} -P RMIT_BlenderDriver.py -- -i {} -e {} -d {} -c {} -l {} -x {}".format(
                        blenderPath,self.filePathIn,self.filePathOut,self.decimateAmount,self.cleanUp,str(self.loose),self.percent)
