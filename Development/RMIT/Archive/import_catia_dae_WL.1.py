...
"""
import fileinput: Not used at present (01/28/19).
"""
# import fileinput


bl_info = {
    "name": "Import Collada from CATIA 2018/2019",
    "author": "Arjun Ayyangar/William Little",
    "version": (1, 0, 0),
    "blender": (2, 80, 0),
    # "blender": (2, 79, 0),
...
"""
    BLENDER/OPERATING SYSTEM interface section: Use standard interface functions to work with Blender system,
    access Catia-generated COLLADA files, and load them into the Blender environment.
"""


def import_collada(path):
    """
    Function import_collada(path): Import COLLADA file specified by input path parameter using defined bpy function
    collada_import.
...
def load(operator, context, **args):
    ...
    ext = parts[1].lower()
    if ext == ".dae":
        import_collada(update_file(filepath))
        ...
        #
        # os.remove(update_file(filepath))
...
class ImportCATIA(bpy.types.Operator, ImportHelper):
    ...
    bl_label = "Import COLLADA from Catia 2019"
...
def register():
    """
    Function register: a function which only runs when enabling this addon - loads the module without activating
    the addon.
    """
    bpy.utils.register_class(ImportCATIA)
...
def unregister():
    """
    Function unregister: a function which unloads anything loaded by register(). Called when the addon is disabled.
    """
    bpy.utils.unregister_class(ImportCATIA)
...
"""
01/29/19: Leave these two functions intact in case they can be used in the future.
